...
if not fetiche.dot_tracker then fetiche.dot_tracker = {} end
if not temp_buffs then temp_buffs = {104509,104510,128985,33702,126577,126478,125487,136082,126605,126734,126476,138898,139133,138786,138703,104993,105702,148897,148906,146184,146046,137590} end
if not powerful_buffs then powerful_buffs = {147357,128985,126577,126478,136082,138898,139133,146046,148906,148897,104509,145164,138963,114206} end
if not SpellLocks then SpellLocks = {143343,138763,137457} end
if not Immunities then Immunities = {116994,122540,123250,106062,110945,143593,143574} end
...
-- Round numbers
function fetiche.PowerRound(num) return math.floor(num+.5) end
function fetiche.Round(number, decimal)
	local multiplier = 10^(decimal or 0)
	return math.floor(number * multiplier + 0.5) / multiplier
end

-- Check for a raid buff
function fetiche.checkRaidBuff(index)
	if not GetRaidBuffTrayAuraInfo(index) then return true end
	return false
end

-- Check if we can cast a spell before buff expires
function fetiche.castcheck(spell, buff)
	...
	if buffName then
	   ...
	   if buffTime - GetTime() > fetiche.Round(select(7,GetSpellInfo(spell))/1000,2) + 0.8 then return true else return false end
	end
	return false
end

-- Calculate DOT power returns
function fetiche.dotPower(spellID)
	...
	if select(2,UnitClass("player")) == "PRIEST" then
		if GetSpecialization() == 3 then
			...
			-- Shadow Word: Pain
			if spellID == 589 then
				ticks		= fetiche.PowerRound(18/tick_every)
				duration	= ticks*tick_every
				damage		= ticks*(623+spd*0.293)*damage_bonus*dmg_buff
				dps			= fetiche.PowerRound(damage/duration)
				dot_power	= fetiche.PowerRound(dps/100)/10
				return dot_power
			end
			
			-- Vampiric Touch
			if spellID == 34914 then
				ticks		= fetiche.PowerRound(15/tick_every)
				duration	= ticks*tick_every
			...
			if spellID == 2944 then
				...
				ticks		= fetiche.PowerRound(5/tick_every)
				duration	= ticks*tick_every
	...
	elseif select(2,UnitClass("player")) == "MAGE" then
		if GetSpecialization() == 1 or GetSpecialization() == 2 or GetSpecialization() == 3 then
			...
			if spellID == 44457 then
				...
				ticks		= fetiche.PowerRound(12/tick_every)
				duration	= ticks * tick_every
	...
	elseif select(2,UnitClass("player")) == "WARLOCK" then
		if GetSpecialization() == 1 then
			...
			-- Agony
			if spellID == 980 then
				ticks		= fetiche.PowerRound(24/tick_every)
				duration	= ticks*tick_every
			...
			if spellID == 30108 then
				ticks		= fetiche.PowerRound(14/tick_every)
				duration	= ticks*tick_every
		...
		elseif GetSpecialization() == 2 then
			...
			if spellID == 603 then
				...
				ticks		= fetiche.PowerRound(60/tick_every)
				duration	= ticks * tick_every
...
function fetiche.tempBuffs(threshold)
...
function fetiche.calculateDot(unit, spell)
	local id = UnitGUID(unit)
	local tracker = fetiche.dot_tracker
	local power = fetiche.dotPower(spell)
	
	-- Shadow Word: Pain
	if spell == 589 then
		local shadowWordPain = select(7,UnitDebuffID(unit,SWP,"PLAYER"))
		local GCD = fetiche.Round(1.5/((UnitSpellHaste("player")/100)+1),2)
		local tick_every = fetiche.Round(3/(1+(UnitSpellHaste("player")/100)),2)
		local max_timer = fetiche.Round(fetiche.PowerRound(18/(3/(1+(UnitSpellHaste("player")/100))))*(3/(1+(UnitSpellHaste("player")/100))),2)
				...
				if tracker[i].guid == id and tracker[i].spellID == spell then
					...
					if UnitBuffID("player",138963) then
						...
						if power > tracker[i].swpPower then
							if (added_ticks >= remaining_ticks) or fetiche.tempBuffs(1+GCD) then return true else return false end
						end
	...
	if spell == 34914 then
		...
		local GCD = fetiche.Round(1.5/((UnitSpellHaste("player")/100)+1),2)
		local tick_every = fetiche.Round(3/(1+(UnitSpellHaste("player")/100)),2)
		local max_timer = fetiche.Round(fetiche.PowerRound(15/(3/(1+(UnitSpellHaste("player")/100))))*(3/(1+(UnitSpellHaste("player")/100))),2)
				...
				if tracker[i].guid == id and tracker[i].spellID == spell then
					...
					if UnitBuffID("player",138963) then
						...
						if power > tracker[i].vtPower then
							if (added_ticks >= remaining_ticks) or fetiche.tempBuffs(tick_every+GCD+2) then
	...
	-- Devouring Plague
	if spell == 2944 then
		local tick_every = fetiche.Round(1/(1+(UnitSpellHaste("player")/100)),2)
...
function fetiche.clip(spell, unit)
...
function fetiche.immunities(unit)
...
function fetiche.interrupts(unit)
	if not unit then return false end
...
function fetiche.validate(unit, spell)
	-- Sanity checks
	if not unit then return false end
	...
	if unit == "mouseover" then if UnitIsUnit("mouseover", "target") then return false end end
	
	if UnitExists(unit) and exists then return true else return false end
end

-- Register library
ProbablyEngine.library.register("fetiche", fetiche)